...
class _OwnerAddProductScreenState extends State<OwnerAddProductScreen> {
  ...
  Future<void> _fetchCategories() async {
    try {
      ...
      final response = await http.get(
        // Uri.parse('${ApiConfig.baseUrl}/kategori'), // Gunakan ini jika ApiConfig sudah siap
  ...
  Future<void> _saveProduct() async {
    ...
    if (_selectedImage == null) {
      _showSnackBar('Silakan upload foto produk terlebih dahulu!', Colors.orange);
      return;
    }
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST',
          // Uri.parse('${ApiConfig.baseUrl}/produk') // Gunakan ini jika ApiConfig sudah siap
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (responseData['status'] == 200 || responseData['status'] == 201) {
          ...
          if (mounted) {
            // Kembali ke halaman sebelumnya dan bawa sinyal 'true' (sukses)
            Navigator.pop(context, true);
          }
        } else {
          _showErrorFromApi(responseData, 'Gagal menambahkan produk');
        }
      } else {
        _showErrorFromApi(responseData, 'Gagal menambahkan produk');
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFFDFDFD), // Warna background putih bersih sesuai desain
      appBar: AppBar(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFFFCECDD), // Oranye sangat muda
  ...
  // Widget Bantuan untuk Label Teks
  ...
  // Widget Bantuan untuk Style Kotak Input
